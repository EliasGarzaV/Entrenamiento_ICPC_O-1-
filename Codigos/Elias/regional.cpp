...
int main() {
    ...
    for (int i = 1; i < perimeter.size() && pos; i++) {
        ...
        if (perimeter[i].second < OFFSET) {
            ...
            cout << "OPEN " << char(perimeter[i].second + 'a') << endl;
        } else if (perimeter[i].second - OFFSET == top.second) {
            cout << "CLOSE " << char(top.second + 'a') << endl;
            check.pop();
        } else {
            pos = 0;
            break;
        ...
        // if (perimeter[i].first > top.second) {
        //     check.pop();

        //     while (!check.empty()) {
        //         if (perimeter[i].first > top.second) {
        //             check.pop();
        //         } else if (perimeter[i].second > top.second) {
        //             pos = 0;
        //             break;
        //         }
        //     }
